    ...
    for mx in mx_records:
        try:
            with smtplib.SMTP(mx) as server:
                ...
                logging.info(
                    f"Batch of {len(to_emails)} emails sent successfully via {mx}"
                )
                break
        except Exception as e:
            logging.error(
                f"Failed to send batch of {len(to_emails)} emails via {mx}: {e}"
            )
...
# Email details
from_email = "noreply@beslist.nl"
from_name = "EnergiePanel"
subject = "Laat je stem horen en win mooie prijzen"
...
with ThreadPoolExecutor(max_workers=5) as executor:
    ...
    for batch in batches:
        futures.append(
            executor.submit(send_email_via_mx, batch, from_email, from_name,
                            subject, html_body))
